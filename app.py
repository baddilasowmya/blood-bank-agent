...
async def grader():
    """
    Run all 3 tasks with the greedy agent and return per-task scores.
    Every score and sub-metric is strictly in (0.001, 0.999).
    """
    def _safe(v: float) -> float:
        return max(0.001, min(0.999, float(v)))

    def _compute(lives_pct: float, steps_used: int, max_steps: int,
                 capacity: int, cap_remaining: int) -> dict:
        ...
        lives_frac = _safe(lives_frac)
        util = _safe(util)
        speed = _safe(speed)
        raw = 0.70 * lives_frac + 0.15 * util + 0.15 * speed
        score = _safe(raw)
    ...
    return {
        ...
        "tasks": tasks_list,
        "breakdown": {"weights": {"lives_saved": 0.70,
                                   "utilization": 0.15, "speed": 0.15}},
    }
...
async def baseline():
    ...
    for task_id, scenario in [
        ...
        results.append({
            ...
            "lives_saved_pct": max(0.001, min(0.999, round(lives_pct / 100.0, 4))),
...
async def _run_greedy_baseline(scenario_name: str) -> tuple:
    ...
    def _s(v): return max(0.001, min(0.999, float(v)))
    max_steps = obs.max_steps
    lives_frac = _s(lives_pct / 100.0)
    utilization = _s((capacity - cap_remaining) / capacity) if capacity > 0 else 0.001
    speed = _s(1.0 - step_count / max_steps) if max_steps > 0 else 0.001
    score = _s(0.7 * lives_frac + 0.15 * utilization + 0.15 * speed)
    return score, lives_pct, step_count
