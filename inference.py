...
def log_step(step: int, action: dict, reward: float, obs: BloodObservation) -> None:
    payload = {
        "step": step,
        "action": action,
        "reward": max(0.001, min(0.999, round(reward, 4))),
        "lives_saved_pct": max(0.001, min(0.999, round(obs.lives_saved_pct / 100.0, 4))),
...
def _compute_score(lives_pct: float, steps_used: int, max_steps: int,
    ...
    utilization = _safe((capacity - cap_remaining) / capacity) if capacity > 0 else 0.001
    speed = _safe(1.0 - steps_used / max_steps) if max_steps > 0 else 0.001
    score = _safe(0.7 * lives_frac + 0.15 * utilization + 0.15 * speed)
    return score
...
async def main() -> None:
    ...
    avg_score = max(0.001, min(0.999, round(sum(r["score"] for r in results) / len(results), 4)))
